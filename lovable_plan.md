...
### 6.3 Планирование и спецификации (до кода)

| Tool / режим | Назначение |
|--------------|------------|
| `studio.plan_task` *(или фаза в промпте)* | Разбить запрос пользователя на шаги: страницы, данные, API, риски; выход — краткий markdown-план в чат + привязка к `StudioTask` |
| `studio.summarize_requirements` | Уточнить неоднозначности (целевая аудитория, обязательные экраны, интеграции БД) |
| Шаблон **spec** в репозитории | `SPEC.md` или поле задачи: цели, не-цели, макеты, брейкпоинты, критерии готовности |

Пайплайн по умолчанию: **план → согласование с пользователем (опционально UI «Применить план») → реализация**.
...
*Версия документа: 1.1 · премиум-планка, стек (React/Tailwind/БД), расширенный инструментарий агента (планирование, тесты, аудит, DoD). Инфраструктура: ollama.site-al.ru / ollama.site-ai.ru.*
